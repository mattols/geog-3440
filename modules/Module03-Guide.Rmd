...
This data is from the 1974 Motor Trend US magazine, and comprises fuel consumption and 10 aspects of automobile design and performance for 32 automobiles (1973–74 models).
...
Another way to summarize data by row and column is to use the `apply` functions. Your book goes over variations of this function. 

In a basic sense, apply can be used to summarize row and column information for example:

```{r}
# summarize by row
mat <- matrix(sample(1:100, 9), nrow=3)
mat
apply(mat, 1, sum) # add all values by row 
```

This can also be used with a dataframe:

```{r}
# average car stats for datasets 
apply(mtcars[, c("mpg","hp", "wt")], 2, mean) # by column
```

See more examples in your book or with `swirl`.
