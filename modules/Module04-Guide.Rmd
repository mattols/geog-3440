...
as.POSIXct(format(now, "%Y-%m-%d %H:00:00")) # format to hourly data and reconvert to datetime format
```
...
Vector arithmetic operations can be useful when exploring data. However, rather than combining vectors or using scalars (e.g. `2 * c(1,3,5,7)`) you can perform arithmetic operations directly *between* neighboring values in a vector:
...
This could be useful if you are interested in decomposing cumulative values or performing other relationships directly between neighbors.
